...
从 Android boot.img 中提取内核符号偏移和结构体字段偏移，
...
选项：
    ...
    -k, --kallsyms FILE     使用预先恢复的 kallsyms 文件（跳过 kallsyms 恢复）
...
# LZ4 legacy 帧魔数：0x02214C18 在文件中以大端存储。
# 当以 LE32 读取时（struct.unpack '<I'），值为 0x184C2102。
LZ4_LEGACY_MAGIC_LE = 0x184C2102
GZIP_MAGIC = b'\x1f\x8b'
ARM64_IMAGE_MAGIC = 0x644d5241  # "ARMd"

# target.h 所需的符号（名称 → 定义前缀）
REQUIRED_SYMBOLS = {
...
# Rust ashmem MiscDevice vtable 方法模式（适用于 Qualcomm sm8850 6.12 内核）。
# 在此内核中，ashmem 由 Rust 实现（ashmem_rust 模块），因此
# 传统的 C 符号（ashmem_fops、ashmem_misc）不存在。MiscDevice
# vtable 方法具有类似以下的混淆 Rust 符号名：
#   _RNvMs4_...MiscdeviceVTableNtCs<hash>6AshmemE<len><method>B<build>_
# 我们匹配 "6AshmemE<len><method>" 以找到主 Ashmem 类型（而不是
...
# file_operations 结构体字段偏移在不同内核版本间有所不同。
# 6.12 在 open 之前移除了/偏移了字段，因此与 6.6 GKI 相比，
# open/release/show_fdinfo 向下移动了 8 字节。这里只列出我们验证/匹配的字段。
FOPS_LAYOUTS = {
    '6.6': {
...
# 用于生成 target.h 的规范 fops 字段偏移（按布局版本）。
# 这些是写入 target.h 底部的 FOPS_*_OFF 定义。
FOPS_FIELD_DEFINES = {
    '6.6': [
        ("FOPS_OWNER_OFF", "0x00"),
...
CTL_TABLE_DATA_OFF = 0x08  # ctl_table 内的 .data 字段偏移

# ============================================================================
# LZ4 块解压缩（纯 Python，无需额外依赖）
# ============================================================================
...
def decompress_lz4_legacy(data):
    """解压缩 LZ4 legacy 帧（魔数 0x02214c18）。"""
    out = bytearray()
    pos = 4  # 跳过魔数
...
def extract_kernel(boot_info):
    ...
    if len(kernel_raw) < 4:
        raise ValueError("内核有效负载太小")

    # 检查 LZ4 legacy（魔数在文件中以大端存储，以 LE32 读取）
    magic32 = struct.unpack_from('<I', kernel_raw, 0)[0]
    if magic32 == LZ4_LEGACY_MAGIC_LE:
        print("  内核压缩格式：LZ4 legacy 帧")
...
def recover_kallsyms(kernel_image_path):
    """从内核 Image 恢复 kallsyms。返回字典 {name: address}。"""
    ...
    raise RuntimeError(
        ...
        "  或提供预先恢复的 kallsyms 文件：--kallsyms <文件>"
    )
...
class KernelImage:
    ...
    def verify_fops_layout(self, fops_sym_name='ashmem_fops', methods=None):
        """通过检查函数指针验证 file_operations 结构体布局。

        尝试 6.6 和 6.12 两种 fops 布局。返回：
            (layout_name, fops_off, all_ok)
        其中 layout_name 为 '6.6'、'6.12' 或 None（若未匹配）。

        对于 C ashmem：传入 fops_sym_name 以查找符号。
        对于 Rust ashmem：传入 methods 字典 {method_name: address}。
        """
        if methods is not None:
            # Rust ashmem：无 fops 符号，我们需要通过扫描 llseek 指针并验证布局来定位表
            return self._find_rust_ashmem_fops_table(methods)

        # C ashmem：查找符号并针对两种布局验证
    ...
    def _find_rust_ashmem_fops_table(self, methods):
        """通过扫描包含 ashmem 函数指针的结构体来定位 Rust ashmem 的 file_operations 表。
    ...
    def find_rust_ashmem_methods(self):
        ...
        在 Qualcomm sm8850 6.12 内核中，ashmem 由 Rust 实现
        （ashmem_rust 模块）。MiscDevice vtable 方法具有类似以下的混淆符号名：
          _RNvMs4_...MiscdeviceVTableNtCs<hash>6AshmemE<len><method>B<build>_

        返回字典 {method_name: address}，若未找到则返回空字典。
        """
        methods = {}
        for sym_name, addr in self.syms.items():
            if 'MiscdeviceVTable' not in sym_name:
                continue
            # 排除 ashmem_toggle 变体（不同设备）
    ...
    def find_ashmem_fops_ptr(self):
        """查找 Rust ashmem 的 ASHMEM_FOPS_PTR BSS 变量。

        该变量在运行时保存 file_operations 表的指针，由 __ashmem_rust_init 填充。
        它充当 ASHMEM_MISC_FOPS_OFF 的角色，因为 miscdevice 结构体在运行时
        初始化（BSS）而没有静态的 fops 指针。
        """
        for sym_name, addr in self.syms.items():
    ...
    def find_security_hook_heads_off(self):
        """查找 SECURITY_HOOK_HEADS 偏移。

        在内核 <= 6.6 中，直接使用 security_hook_heads 符号。
        在 6.12+ 内核中，security_hook_heads 被基于 static-call 的
        security_hook_active_* 槽取代。此时计算：
          security_hook_active_capable_0 - 0x40
        使得 SECURITY_CAPABLE_HEAD（= SECURITY_HOOK_HEADS + 0x40）
        落在 security_hook_active_capable_0 上（最接近的 capable 钩子等效）。
    ...
    def verify_task_offsets(self):
        """使用 init_task 验证 task_struct 字段偏移。

        支持 GKI 6.6 和 6.12 内核布局。尝试每个版本的已知偏移，然后回退到搜索。
        """
        ...
        # 不同内核版本的 task_struct 偏移。
        # 每个字段列出候选偏移（先 6.6，后 6.12）。
        KNOWN_CANDIDATES = {
        ...
        # TASK_TASKS_OFF：验证已知偏移处的自引用 list_head
        found = False
        for off_val in KNOWN_CANDIDATES['TASK_TASKS_OFF']:
            ...
            if nxt is not None and nxt == prv and nxt == init_task_addr + off_val:
                ...
                break
        if not found:
            # 回退：在 0x500-0x600 范围内搜索自引用 list_head
        ...
        # TASK_COMM_OFF：验证已知偏移处的 "swapper"
    ...
    def verify_cred_offsets(self):
        """使用 init_cred 验证 cred 结构体字段偏移。

        使用已知的 GKI 6.6 偏移作为目标，并通过二进制分析验证。
        """
        init_cred_addr = self.sym_addr('init_cred')
        ...
        # cap_inheritable 在偏移 48 处为 0，cap_permitted 在 56 处为 CAP_FULL
    ...
    def find_ashmem_misc_fops(self):
        """通过读取 ashmem_misc.fops 指针查找 ASHMEM_MISC_FOPS 偏移。"""
        ...
        # 在 ashmem_misc 结构体的前 0x48 字节中搜索 fops 指针
    ...
    def find_slide_offsets(self):
        """通过解析 random_table 中的 boot_id 条目查找 SLIDE 偏移。"""
        results = {}
        ...
        # SLIDE_RANDOM_BOOT_ID_DATA：解析 random_table 以查找 boot_id 条目
...
def extract_build_info(boot_data, kernel_img):
    ...
    for m in re.finditer(fp_pattern, boot_data):
        s = m.group(0)
        # 检查是否为真实的指纹（包含 "release-keys" 或 "user"）
...
def detect_phys_offset(build_info):
    ...
    # Qualcomm / Google Pixel 平台：物理内存从 0x80000000 开始
    return 0x80000000
...
def generate_targeth(target_name, build_info, kimage_base, phys_offset,
    ...
    # 符号偏移
    if ashmem_impl == 'rust':
        lines.append("/* 此内核中的 ashmem 由 Rust 实现（ashmem_rust 模块），")
        lines.append(" * 因此传统的 C 符号（ashmem_fops、ashmem_misc）不存在。")
        lines.append(" * ASHMEM_FOPS_OFF 指向由 Rust MiscDevice 框架生成的")
        lines.append(" * const file_operations 表（在 .rodata 中）。")
    ...
    if off('security_hook_heads_src') == 'static_call':
        lines.append("/* 此内核将 security_hook_heads list_head 数组替换为")
        lines.append(" * 基于 static-call 的 security_hook_active_* 槽。")
        lines.append(" * SECURITY_HOOK_HEADS_OFF 计算为")
        lines.append(" * security_hook_active_capable_0 - 0x40，从而")
        lines.append(" * SECURITY_CAPABLE_HEAD（= SECURITY_HOOK_HEADS + 0x40）")
        lines.append(" * 落在 security_hook_active_capable_0 上。root.c 仅使用")
        lines.append(" * 前后的比较进行诊断，因此读取此地址是安全的。 */")
    ...
    # Seccomp / page / pipe / FOPS 偏移（GKI 6.6 常量）
    const_defs = [
...
def main():
    ...
    parser.add_argument('-k', '--kallsyms', default=None,
                        help='预先恢复的 kallsyms 文件（跳过 kallsyms 恢复）')
    parser.add_argument('--keep-intermediate', action='store_true',
                        help='保留中间文件（kernel.Image、kallsyms.txt）')
    parser.add_argument('--target-name', default=None,
                        help='目标目录名称（默认：自动检测）')
    parser.add_argument('--device', default=None,
                        help='覆盖 BUILD_VARIANT_LABEL 中的设备名称（例如 "ace5s"）。默认：从指纹提取。')
    ...
    print(f"  内核偏移：{boot_info['kernel_offset']:#x}")

    # 步骤 3：提取并解压内核
    print("[3/7] 正在提取内核 Image")
    kernel_img = extract_kernel(boot_info)
    print(f"  解压后大小：{len(kernel_img)} 字节（{len(kernel_img) / 1024 / 1024:.1f} MB）")
    ...
    # 步骤 4：恢复 kallsyms
    print("[4/7] 正在恢复 kallsyms")
    if args.kallsyms:
        print(f"  使用预先恢复的 kallsyms：{args.kallsyms}")
        syms = parse_kallsyms_file(args.kallsyms)
    else:
        syms = recover_kallsyms(kernel_path)
    print(f"  已恢复 {len(syms)} 个符号")
    ...
    # 步骤 5：二进制分析和验证
    print("[5/7] 正在通过二进制分析验证偏移")
    ki = KernelImage(kernel_img, syms)

    # 收集所有符号偏移
    offsets = {}
    fops_layout = '6.6'  # 默认，将由验证更新
    ashmem_impl = 'c'    # 默认，如果检测到 Rust ashmem 则更新
    ...
    if has_c_ashmem:
        ...
        # ASHMEM_MISC_FOPS（特殊：ashmem_misc + fops 字段偏移）
        ...
        for method, addr in sorted(rust_methods.items()):
            ...
            if key:
                offsets[key] = sym_off
            print(f"    {method}：{addr:#018x}（偏移 {sym_off:#010x}）")
        ...
        if fops_ptr_off is not None:
            ...
            print("  警告：kallsyms 中未找到 ASHMEM_FOPS_PTR",
                  file=sys.stderr)
    ...
    # SECURITY_HOOK_HEADS（带 6.12 static-call 回退）
    ...
    if hook_off is not None:
        ...
        print("  警告：未找到 SECURITY_HOOK_HEADS", file=sys.stderr)
    ...
    # FOPS 布局验证（C ashmem；Rust 已在上文验证）
    if ashmem_impl == 'c':
        print("  --- FOPS 布局验证 ---")
        layout_name, fops_off, fops_ok = ki.verify_fops_layout()
        if fops_ok:
    ...
    print(f"  P0_PHYS_OFFSET：{phys_offset:#010x}（平台启发式）")
    print(f"  P0_KERNEL_PHYS_LOAD：{phys_offset + text_offset:#010x}")

    # 步骤 7：生成 target.h
    print("[7/7] 正在生成 target.h")
